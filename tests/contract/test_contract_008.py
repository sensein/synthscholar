...
class TestAssemblePrismaReviewSignature:
    def test_function_is_async_coroutine(self):
        from synthscholar.pipeline import assemble_prisma_review
...
class TestAssemblePrismaReviewWave1Timeout:
    async def test_wave1_hang_raises_timeout_error(self):
        ...
        with (
            patch("synthscholar.pipeline._assemble_methods", return_value=MagicMock()),
            patch("synthscholar.pipeline._assemble_extracted_studies", return_value=[]),
            patch("synthscholar.pipeline.run_thematic_synthesis", _hanging),
            patch("synthscholar.pipeline.run_introduction_section", AsyncMock(return_value=MagicMock())),
            patch("synthscholar.pipeline.run_quantitative_analysis", AsyncMock(return_value=None)),
        ):
    ...
    async def test_wave1_introduction_hang_raises_timeout_error(self):
        ...
        with (
            patch("synthscholar.pipeline._assemble_methods", return_value=MagicMock()),
            patch("synthscholar.pipeline._assemble_extracted_studies", return_value=[]),
            patch("synthscholar.pipeline.run_thematic_synthesis", AsyncMock(return_value=MagicMock())),
            patch("synthscholar.pipeline.run_introduction_section", _hanging),
            patch("synthscholar.pipeline.run_quantitative_analysis", AsyncMock(return_value=None)),
        ):
            with pytest.raises(asyncio.TimeoutError):
                await assemble_prisma_review(
...
class TestAssemblePrismaReviewWave2Timeout:
    async def test_wave2_hang_raises_timeout_error(self):
        ...
        with (
            patch("synthscholar.pipeline._assemble_methods", return_value=MagicMock()),
            patch("synthscholar.pipeline._assemble_extracted_studies", return_value=[]),
            patch("synthscholar.pipeline.run_thematic_synthesis", AsyncMock(return_value=mock_synthesis)),
            patch("synthscholar.pipeline.run_introduction_section", AsyncMock(return_value=MagicMock())),
            patch("synthscholar.pipeline.run_quantitative_analysis", AsyncMock(return_value=None)),
            patch("synthscholar.pipeline.run_abstract_section", _hanging),
            patch("synthscholar.pipeline.run_discussion_section", AsyncMock(return_value=MagicMock())),
            patch("synthscholar.pipeline.run_conclusion_section", AsyncMock(return_value=MagicMock())),
        ):
            with pytest.raises(asyncio.TimeoutError):
                await assemble_prisma_review(
                    _make_mock_result(), MagicMock(), assemble_timeout=0.1
